...
$meta_boxes[] = array(
  'id' => 'bbp',
	...
	'fields' => array(
		...
		// adding option for responsive background image
		array(
			'name'		=> 'Image Sizing',
			'id'		=> "{$prefix}sizing",
			'type'		=> 'radio',
			...
			'std'		=> 'cover',
			'desc'		=> 'Set the background image to be scaled to 100% or responsive?'
		),
	)
);
	...
	if($src != ''){
		...
		?><style type="text/css">
			...
			<?php echo $element; ?>, <?php echo $element; ?>.custom-background  { 
				...
				<?php } ?>
				<?php if($sizing != '') { ?>
					background-size: <?php echo $sizing; ?>;
				<?php } ?>
...
function get_data($id){
	...
	$sizing = get_post_meta( $id, '_bpp_sizing', true);
	$data = array();
